...
export async function getUserConnections({
  ...
  let ids = profileUser[type].map(String);

  // move logged-in user to top (if present)
  if (viewerId) {
    const viewerIndex = ids.indexOf(String(viewerId));
    if (viewerIndex > -1) {
      ids.splice(viewerIndex, 1); // remove from current position
      ids.unshift(String(viewerId)); // add to top
    }
  }

  // Cursor filtering
  const startIndex = cursor ? Math.max(ids.indexOf(cursor) + 1, 0) : 0;
